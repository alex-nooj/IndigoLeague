...
class SuccessCallback(sb3_callbacks.BaseCallback):
    def __init__(self, league_dir: pathlib.Path, tag: str, verbose: typing.Optional[int] = 1):
    ...
    def _on_step(self) -> bool:
        if self._next_step:
            ...
            self.logger.record(f"trueskill/mu", self.training_env.envs[0].env.matchmaker.agent_skills[self.training_env.envs[0].env.tag].mu)
            self.logger.record(f"trueskill/sigma", self.training_env.envs[0].env.matchmaker.agent_skills[
                self.training_env.envs[0].env.tag].sigma)
