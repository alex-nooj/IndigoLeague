...
from utils.smogon_data import create_pokemon_str


def generate_all_pokemon() -> typing.Dict[str, typing.List[str]]:
    data = SmogonData()
    pokemon = {}
    filename = pathlib.Path(__file__).parent.parent / "utils/data/transfer_only_moves.json"
    ...
    for mon in tqdm.tqdm(mons):
        if mon.lower() != "slowbro":
            continue
        pokemon[mon] = []
        moves = [m for m in data.smogon_data["data"][mon]["Moves"] if m not in ["", "zapcannon"]]
        if mon.lower() in transfer_only_moves:
            moves = [m for m in moves if format_str(m) not in transfer_only_moves[format_str(mon)]]
                ...
                for k in range(j+1, len(moves)-1):
                    for l in range(k+1, len(moves)):
                        ...
                        for ability in data.smogon_data["data"][mon]["Abilities"]:
                            item = list(data.smogon_data["data"][mon]["Items"].keys())[0]
                            spread = list(data.smogon_data["data"][mon]["Spreads"].keys())[0]
...
class ExtensiveTeamBuilder(teambuilder.Teambuilder):
    def __init__(self, mons_list: typing.Dict[str, typing.List[str]], filename: pathlib.Path):
        self.mons = mons_list
